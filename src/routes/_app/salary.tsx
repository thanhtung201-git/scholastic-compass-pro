 ...
 * SalaryPage.tsx
 * ---------------------------------------------------------------
 * Menu: Salary → Bảng Lương Và Cam Kết Academy
 *
 ...
 *
 * Features:
 *   - Auto-generate receipts for all employees for a chosen period
 *   - Paginated payroll table
 *   - "Phiếu chi" → A4 payslip modal with print/PDF
 *   - "Duyệt chi" → sets payment_status = 'Đã chi'
 * ---------------------------------------------------------------
 */
...
async function fetchReceipts(month: number, year: number): Promise<PayrollRow[]> {
  ...
  if (error) {
    console.error("fetchReceipts:", error);
    return [];
  }
...
/**
 * Generate receipts for all employees that don't yet have one
 * for the selected period. The DB trigger calculates all amounts.
 */
async function generateReceipts(month: number, year: number): Promise<number> {
  ...
  // 2. Find which already have a receipt this period
  ...
  // 3. Insert only the missing ones (trigger fills all computed columns)
  const toInsert = employees
    ...
    .map((e: any) => ({
      ...
      period_month: month,
      period_year: year,
      // Placeholder values — the BEFORE INSERT trigger overwrites them
      gross_salary: 0,
      insurance: 0,
      tax: 0,
      net_salary: 0,
  ...
  const { error: insErr } = await supabase.from("payroll_receipt").insert(toInsert);

  if (insErr) {
    console.error("generateReceipts:", insErr);
    return 0;
  }
...
function PayslipModal({ row, onClose }: { row: PayrollRow; onClose: () => void }) {
  const grossMinusIns = row.gross_salary - row.insurance;
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3 bg-gray-50 border-b border-gray-100">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-gray-200 text-gray-400 hover:text-gray-700 transition-colors text-base"
            >
              ×
            </button>
          </div>
        ...
        <div id="payslip-print" className="px-10 py-8">
          ...
          <div className="text-center mb-7">
            <p className="text-[15px] font-extrabold tracking-[0.18em] text-gray-900 uppercase">
              MCNA Technology School
            </p>
            <p className="text-[10px] tracking-[0.3em] text-blue-500 font-semibold mt-0.5 uppercase">
              Khoa học · Công nghệ · Kỹ thuật
            </p>
            <p className="text-[11px] text-gray-400 mt-1">
              Cầu Giấy, Hà Nội &bull; Hotline: 1900-xxxx
            </p>
            <div className="mt-3 mx-auto w-12 border-b-2 border-blue-400 rounded-full" />
          </div>

          <div className="text-center mb-6">
            <p className="text-sm font-bold uppercase tracking-[0.15em] text-gray-800">
              Phiếu Chi Lương &amp; Giờ Thỉnh Giảng
            </p>
            <p className="text-xs text-gray-400 mt-1">
              Chu kỳ thanh toán: Tháng {periodLabel(row.period_month, row.period_year)}
            </p>
          </div>
          ...
          <div className="flex justify-between text-[13px] mb-6 pb-3 border-b border-dashed border-gray-200">
            <span className="text-gray-500">
              ...
              <span className="font-bold text-gray-800">
                {row.employee_id.slice(0, 8).toUpperCase()}
              </span>
            </span>
            <span className="text-gray-500">
              Họ tên: <span className="font-bold text-gray-800">{row.full_name}</span>
            </span>
          </div>
          ...
          <div className="pt-4 border-t-2 border-gray-800 flex justify-between items-center">
            <span className="text-sm font-extrabold text-blue-700 uppercase tracking-wide">
          ...
          <p className="text-center text-[10px] text-gray-300 mt-7">
            Phiếu lương đã ký số tự động bởi MCNA School ERP. Xin cảm ơn nỗ lực giảng dạy của thầy
            cô!
          </p>
        </div>
      </div>
...
function StatusBadge({ status }: { status: PayrollRow["payment_status"] }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-3 py-1 rounded-md text-xs font-semibold border ${cfg}`}
    >
      {status}
    </span>
  );
...
function SalaryPage() {
  ...
  const handleApprove = async (row: PayrollRow) => {
    ...
    if (ok) {
      ...
      showToast(`✅ Đã duyệt chi cho ${row.full_name}`);
    } else {
      showToast("❌ Duyệt chi thất bại. Vui lòng thử lại.");
    }
    setApprovingId(null);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      {/* Toast */}
      {toast && (
        <div className="fixed top-5 right-5 z-[100] bg-white border border-gray-200 shadow-xl rounded-xl px-5 py-3 text-sm text-gray-800 animate-[fadeSlide_0.25s_ease]">
          {toast}
        </div>
      )}
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Bảng quyết toán học phí quy đổi giờ thỉnh giảng
            </p>
          </div>

          {/* Controls: period picker + generate button */}
          <div className="flex items-center gap-2">
            {/* Month */}
            <select
              value={month}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 mb-6 grid grid-cols-2 md:grid-cols-4 gap-6 shadow-sm">
          <div>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">
              Tổng Định Mức Quỹ Lương
            </p>
            <p className="text-lg font-extrabold text-gray-900">
              ~{(totalFund / 1e9).toFixed(2)} tỷ đ
            </p>
          </div>
          <div>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/70">
                  {[
                    { label: "Cán bộ / Giáo thọ", align: "left" },
                    { label: "Chu kỳ", align: "right" },
                    { label: "Lương Gross", align: "right" },
                    { label: "Bảo hiểm (10.5%)", align: "right" },
                    { label: "Thuế TNCN", align: "right" },
                    { label: "Lương thực lĩnh", align: "right" },
                    { label: "Trạng thái", align: "center" },
                    { label: "", align: "right" },
                    { label: "Chứng từ", align: "right" },
                  ].map((h, i) => (
                    <th
                      key={i}
                      className={`px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-400 text-${h.align}`}
                    >
                      {h.label}
                    </th>
                  ))}
                ...
                {paginated.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="text-center py-16 text-gray-400 text-sm">
                      Chưa có phiếu lương nào kỳ {periodLabel(month, year)}. Nhấn &ldquo;Chạy bảng
                      tính toán&rdquo; để tạo mới.
                    </td>
                  </tr>
                ) : (
                  paginated.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 text-right text-blue-500 font-medium">
                        {periodLabel(row.period_month, row.period_year)}
                      </td>
                      <td className="px-4 py-3.5 text-right text-gray-800">
                        {fmtVND(row.gross_salary)}
                      </td>
                      <td className="px-4 py-3.5 text-right text-red-500 font-medium">
                        {fmtVND(row.insurance)}
                      </td>
                      <td className="px-4 py-3.5 text-right text-red-500 font-medium">
                        {fmtVND(row.tax)}
                      </td>
                      <td className="px-4 py-3.5 text-right text-blue-600 font-bold">
                        {fmtVND(row.net_salary)}
                      </td>
                      ...
                      <td className="px-2 py-3.5 text-right">
                        {row.payment_status !== "Đã chi" && (
                          ...
                          >
                            {approvingId === row.receipt_id ? "..." : "Duyệt chi"}
                          </button>
                        )}
                      </td>
          ...
          {!loading && rows.length > 0 && (
            <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 bg-gray-50/50">
              <p className="text-xs text-gray-500">
                Đang xem {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, rows.length)}{" "}
                trong tổng số {rows.length} bản ghi lương
              </p>
              <div className="flex items-center gap-4">
                <button
                  onClick={() => setPage((p) => Math.max(1, p - 1))}
                  disabled={page === 1}
                  className="text-xs font-semibold text-gray-500 hover:text-gray-900 disabled:opacity-30 transition-colors"
                >
                  Trước
                </button>

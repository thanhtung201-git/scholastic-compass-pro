...
guides.forEach(guide => {
  ...
  content += '> **Module:** ' + guide.module + '\\n';
  content += '> **Section:** ' + guide.section + '\\n';
  content += '> **Access Levels:** ' + guide.roles + '\\n\\n';
  content += '---\\n\\n';
  content += '## 🌐 Overview\\n' + guide.overview + '\\n\\n';
  content += '## ✨ Key Features\\n';
  
  guide.features.forEach(f => {
    content += '- ' + f + '\\n';
  });
  ...
  fs.writeFileSync(path.join(outputDir, guide.filename), content.replace(/\\\\n/g, '\\n'), 'utf8');
});
